var pr0m31h3u5 = function(t) {
	...
	var e = {
		...
		render: function(t, e) {
			...
			i.style.backgroundColor = t.settings.background,
				i.style.color = t.settings.color, i.style.fontWeight = t.settings.weight, (t.rows != r || t.cols != o) && (o = t.cols, r = t.rows, n.length = 0);
			...
			for (let l = 0; l < r; l++) {
				...
				for (let n = 0; n < o; n++) {
					const o = e[n + r];
					if (o.beginHTML && (u && (h += "</span>", d = {}, u = !1), h += o.beginHTML), !s(o, d)) {
						u && (h += "</span>");
						...
						let a = "";
						e && (a += "color:" + e + ";"),
							n && (a += "background:" + n + ";"), r && (a += "font-weight:" + r + ";"), a && (a = ' style="' + a + '"'), h += "<span" + a + ">", u = !0
					}
					h += o.char, d = o, o.endHTML && (u && (h += "</span>", d = {}, u = !1), h += o.endHTML)
				}
				u && (h += "</span>"),
					i.childNodes[l].innerHTML = h
			}
		}
	};
	const n = [];
	...
	function a(t, e) {
		return "object" == typeof t && ("object" == typeof e && (t.char === e.char && (t.weight === e.weight && (t.color === e.color && t.background === e.background))))
	}

	function s(t, e) {
		return t.weight === e.weight && (t.color === e.color && t.background === e.background)
	}
	var i = {
		...
		render: function(t, e) {
			const n = t.settings.element,
				...
				c = t.settings;
			c.canvasSize ? (n.width = c.canvasSize.width * o, n.height = c.canvasSize.height * o, n.style.width = c.canvasSize.width + "px", n.style.height = c.canvasSize.height + "px") : (n.width = t.width * o, n.height = t.height * o);
			...
			if (g.fillStyle = d, g.fillRect(0, 0, n.width, n.height), g.save(), g.scale(o, o), c.canvasOffset) {
			...
			for (let t = 0; t < a; t++)
				for (let n = 0; n < r; n++) {
					const o = e[t * r + n],
						a = n * i,
						s = t * l;
					o.background && o.background != d && (g.fillStyle = o.background || d, g.fillRect(Math.round(a), s, Math.ceil(i), l)), g.font = (o.weight || f) + h,
						g.fillStyle = o.color || u, g.fillText(o.char, a, s)
				}
			g.restore()
		}
	};
	class l {
		...
		update(t) {
			return this.frames++,
				t >= this.ptime + 1e3 && (this.fps = 1e3 * this.frames / (t - this.ptime), this.ptime = t, this.frames = 0), this.fps
		}
	}
	var c = function(t, e) {
			try {
	...
	function f(t, e, n = {}) {
		return new Promise((function(o) {
			const r = { ...u,
				...
				s = "currentState";
			let i;
			r.restoreState && (h(s, a), a.cycle++), r.element ? "canvas" == r.renderer ? "CANVAS" == r.element.nodeName ? i = d[r.renderer] : console.warn("This renderer expects a canvas target element.") : "CANVAS" != r.element.nodeName ? i = d[r.renderer] : console.warn("This renderer expects a text target element.") : (i = d[r.renderer] || d.text, r.element = document.createElement(i.preferredElementNodeName), document.body.appendChild(r.element));
			const f = [],
				m = {
					x: 0,
					y: 0,
			...
			r.element.addEventListener("pointermove", (t => {
					...
					m.x = t.clientX - e.left, m.y = t.clientY - e.top, f.push("pointerMove")
				})),
				...
				r.allowSelect || ((w = r.element).style.userSelect = "none", w.style.webkitUserSelect = "none", w.style.mozUserSelect = "none", w.dataset.selectionEnabled = "false"), document.fonts.ready.then((e => {
					let o = 3;
					! function e() {
						--o > 0 ? requestAnimationFrame(e) : function() {
							M = p(r.element);
							const e = g(a, r, M, b);
			...
			function N(e) {
				...
				if (m.px = m.x, m.py = m.y, m.ppressed = m.pressed, A != h.cols || E != h.rows) {
					A = h.cols, E = h.rows, x.length = h.cols * h.rows;
					for (let t = 0; t < x.length; t++) x[t] = { ...y,
						char: " "
					}
				}
				if ("function" == typeof t.pre && t.pre(h, d, x, n), "function" == typeof t.main)
					for (let e = 0; e < h.rows; e++) {
						...
						for (let r = 0; r < h.cols; r++) {
							...
							x[a] = "object" == typeof s && null !== s ? { ...x[a],
								...s
							} : { ...x[a],
								char: s
				...
				for ("function" == typeof t.post && t.post(h, d, x, n), i.render(h, x, r); f.length > 0;) {
					const e = f.shift();
	...
	function p(t) {
		const e = getComputedStyle(t),
			...
			a = ("CANVAS" == t.nodeName ? t : document.createElement("canvas")).getContext("2d");
		a.font = r + "px " + n;
		const s = a.measureText("".padEnd(10, "x")).width / 10,
			i = {
				...
				_update: function() {
					const e = p(t);
					for (var n in e) "number" != typeof e[n] && "string" != typeof e[n] || (i[n] = e[n])
				}
			};
		return i
	}
	...
	function E(t, e = 0) {
		...
		for (const t of n) {
			const n = t.split(" ");
			let s = 0;
			for (const t of n) 0 == s ? (o += t, s = t.length, r = Math.max(r, s)) : s + 1 + t.length <= e ? (o += " " + t, s += t.length + 1, r = Math.max(r, s)) : (o += "\n" + t, s = t.length + 1, a++);
			o += "\n", a++
		}
	...
	class N {
		constructor(t, e) {
			this.width = t,
				this.height = e, this.data = new Array(t * e).fill(0)
		}
		setBmp(t, e, n) {
		...
		get(t, e) {
			return t < 0 || t >= this.width || e < 0 || e >= this.height ? 0 : this.data[t + e * this.width]
		}
		sample(t, e) {
			const n = t * this.width - .5,
				o = e * this.height - .5;
			let r = Math.floor(n),
				a = Math.floor(o),
				s = r + 1,
		...
		print() {
			...
			for (let e = 0; e < this.height; e++) {
				for (let n = 0; n < this.width; n++) t += this.get(n, e) < .5 ? "K" : "T";
				t += "\n"
			}
			return t
		}
		pack() {
			...
			for (let n = 0; n < e; n++) {
				let e = 0;
				for (let t = 0; t < 32; t++) this.data[t + 32 * n] > .5 && (e = L(e, t));
				t.push(e)
			}
			return t
		}
	...
	}(q), q.unshift("cvb"), q.unshift("dfg"), q.unshift("ert");
	...
	function _(t, e) {
		...
		if (t.frame % 2 == 1) {
			for (let t = 0; t < 3; t++) 0 == V[t] ? D[t] != R[t] && (D[t] = (D[t] + 1) % F.length) : V[t]--;
			for (let t = 0; t < 3; t++) {
				const e = (H + 0) * t,
					n = 0,
					o = D[t];
	...
	Z.push("  "), Z.push("+ "), Z.push(" ."), Z.push("+ "), Z.push(" ,"), Z.push("· "), Z.push(": "), Z.push("• ");
	const G = " .,·-•─~+:;=*π’“”!?#$@aàbcdefghijklmnoòpqrstuüvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ│0123456789%()".split(""),
		Q = " .·•-+=:;*ABC0123!*".split(""),
		tt = " ·-•~+:*abcXYZ*".split("");
	let et = J(K() * Z.length);
	Q[Q.length - 1] = Z[et][0], tt[tt.length - 1] = Z[et][1];
	for (const t of Z) - 1 == G.indexOf(t[0]) && G.push(t[0]), -1 == G.indexOf(t[1]) && G.push(t[1]);
	...
	function st(t) {
		...
		return void 0 === t.a ? "#" + e + n + o : "#" + e + n + o + Math.round(255 * t.a).toString(16).padStart(2, "0")
	}
	...
	const lt = [it(43690), it(11141120), it(11162880), it(5592575), it(5635925), it(5636095), it(16733525), it(16733695), it(16777215)],
		ct = {
			a: at(160, 160, 160),
			b: at(135, 135, 135)
		},
		...
		dt = function() {
			...
			return function(x, v) {
				return o = Math.floor(x), r = Math.floor(v), a = x - o, s = v - r, i = (o + t) % t, l = (i + 1 + t) % t, c = (r + t) % t, h = (c + 1 + t) % t, d = e[n[n[i] + c]], u = e[n[n[l] + c]], f = e[n[n[i] + h]],
					g = e[n[n[l] + h]], p = M(0, 1, a), m = M(0, 1, s), w = b(d, u, p), y = b(f, g, p), b(w, y, m)
			}
		}(),
		ut = {
			colorA: 0,
	...
	let pt, mt, wt = gt.property("--font-size-change");
	...
	"ontouchstart" in window ? document.addEventListener("touchstart", (function(t) {
		t.target.href && (t.preventDefault(), location.href = t.target.href)
	})) : document.addEventListener("mousedown", (function(t) {
		0 === t.button && t.target.href && (t.preventDefault(), "_blank" == t.target.target ? window.open(t.target.href) : location.href = t.target.href)
	}));
	var yt = Object.freeze({
		...
		pre: function(t, e, n, o) {
			...
			if (wt != r && (t.metrics._update(), wt = r), pt != t.cols || mt != t.rows) {
				pt = t.cols, mt = t.rows;
				const e = pt * mt;
				for (let t = 0; t < e; t++) ut.data[t] = {
					fade: 0,
					heat: 0,
					curr: 0
				}
			}
			if ("white" == o.color) ut.colorA = "lightgray",
				ut.colorB = "white";
			else if (o.color) t.frame % 2e3 == 0 && (ct.a = ot(lt), ct.b = ot(lt)), ht.a = rt(ht.a, ct.a, .01), ht.b = rt(ht.b, ct.b, .01), ut.colorA = st(ht.a), ut.colorB = st(ht.b);
			else {
				...
				ut.radiusSq *= .8, ut.radiusSq = I(ut.radiusSq + .3 * t, 10);
				const n = t > .1 ? .008 : 0;
				if (ut.movAmt = I(ut.movAmt + n, 1), ut.movAmt < .3)
					for (ht.a = ot(lt), ht.b = ot(lt); ht.b == ht.a;) ht.b = ot(lt);
				const o = function(t, e, n) {
					const o = w((n - t) / (e - t), 0, 1);
					return o * o * o * (o * (6 * o - 15) + 10)
				}(.3, .8, ut.movAmt);
				ut.colorA = st(rt(ct.a, ht.a, o)), ut.colorB = st(rt(ct.b, ht.b, o))
			}
			"screensaver" == o.mode && t.frame % 3e3 == 0 && (et = J(K() * Z.length)), 1 == ut.movAmt && (et = J(K() * Z.length));
			...
			ut.movAmt = U(ut.movAmt - .003, 0);
			const i = t.cols < 80 ? 1.3 : .8;
			ut.scale += .01 * (i - ut.scale);
			...
			let g, p, m, b, y, x, M;
			for (let t = 0; t < mt; t++)
				for (let e = 0; e < pt; e++) g = h * (e - .5 * pt) + .5, p = d * (t - .5 * mt) + .5,
					m = g + .5 * (dt(g * f + u, p * f) - .5), b = p + 1.8 * (dt(g * f, p * f + u) - .5), y = Math.floor(m * P.width), x = Math.floor(b * P.height), M = e + t * pt, ut.data[M].tex = U(P.sample(m, b), P.get(y, x))
		},
		main: function(t, e, n, o, r) {
			...
			return {
				...
				color: s >= .99 ? ut.colorA : ut.colorB
			}
		},
		post: function(t, e, n, o) {
				...
				for (let e of n)
					...
					const n = E(e.text, c),
						o = A({
							text: n.text,
							aheat: 1
						}, h, d, ut.data, t.cols, t.rows);
					for (const t of o.wrapInfo) t.first.beginHTML = e.beginHTML, t.last.endHTML = e.endHTML;
					h = o.offset.col + 1, d = o.offset.row, s = U(s, n.numLines)
				}
				a.push({
					x: o,
			...
			for (const e of a) "rows" == e.displayMode ? k({
				char: "─"
			}, e.x, e.y - 1, e.width, 1, ut.data, t.cols, t.rows) : "cols" == e.displayMode && k({
				char: "│"
			}, e.x - 1, 0, 1, s, ut.data, t.cols, t.rows);
			for (let e = 0; e < n.length; e++) {
				const o = ut.data[e];
				o.char && (o.heat = Math.max(o.heat, o.aheat || 0), 1 == o.heat ? o.curr != nt[o.char] ? (o.char = G[o.curr], (t.frame + e) % 2 == 0 && (o.curr = (o.curr + 1) % G.length)) : o.heat = 0 : o.heat > 1 && (o.heat--, o.char = G[o.curr]), n[e] = { ...o
				}, delete ut.data[e].char, delete ut.data[e].beginHTML, delete ut.data[e].endHTML, delete ut.data[e].color)
			}
		}
	});
	return t.boot = function(t) {
		...
		f(yt, {
			...
			"f" == t.key && (e.requestFullscreen ? document.fullscreenElement ? document.exitFullscreen() : e.requestFullscreen() : e.webkitRequestFullscreen && (document.webkitFullscreenElement ? document.webkitExitFullscreen() : e.webkitRequestFullscreen()))
		}))
